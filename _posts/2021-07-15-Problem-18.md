...
The expression above is the [KL-divergence](https://www.countbayesie.com/blog/2017/5/9/kullback-leibler-divergence-explained) between q and \( P\left(t_{i}=j \mid x_{i}, \Theta\right) \) which can be shown to be minimum when \(  q = P\left(t_{i}=j \mid x_{i}, \Theta\right) \).

Furthermore, using [Bayes' Theorem](https://en.wikipedia.org/wiki/Bayes%27_theorem) it can be seen that 
...
- M-step : This essentially involves a parameter update for a given distribution q. I am skipping the full derivation since the update formulae are pretty intuitive. We need to update the weights \( \alpha_{j} \), mean vectors and the covariance matrices. The [formulae](https://biarnes-adrien.medium.com/em-of-gmm-appendix-m-step-full-derivations-4ae95cdd40c9) are given below:
