...
Well, so we need to notice a few things here. First, the apex angle of the triangle is $$ 20 \textdegree $$  which automatically makes the  $$ \delta CDB $$ an isosceles triangle. So, we want to make sure we use that property. Also, not that the base angle EAB $$ = 70 \textdegree $$ which means we can split it into $$ 60 \textdegree + 10 \textdegree $$ and $$ 10 \textdegree $ compliments the base angles $$ 80 \textdegree $$. So, let us go ahead and draw a right angled $$ \delta AGB $$. Everything is constructed below (Sorry for the absence of padding).
