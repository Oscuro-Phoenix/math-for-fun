...
To close the problem, we note that the final expression for $$P\left(\mathbf{y} \mid M_i\right)$$ had the determinant of $$H$$, $$\|H\|$$ in the denominator (Pardon my abuse of notation - norm (\|\|) and det() of a matrix have the same meaning here). This can now we replaced by the determinant of $$\mathcal{I}.n$$ that is $$n^{N_p}\|\mathcal{I}\|$$. 
