...
We see the beautiful Inclusion-Exclusion Principle in action here, when the multiples of \( p_{1} \) and multiples of \( p_{2} \) are subtracted we also make the error of subtracting the multiples of both twice. So, we amend our mistake by adding that \( \frac{n}{p_{1}.p_{2}} \)term.
