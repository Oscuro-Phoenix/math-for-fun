...
Also, since $$ \mathbf{W} $$ and $$ \mathf{y}_{2} $$ are uncorrelated, we may directly write that $$ \operatorname{Var}\left(\mathbf{x}_{1} | \mathf{x}_{2} \right) $$ is the same as
$$ \operatorname{Var}\left( \mathbf{W} \right) $$.

The variance of $$ \mathf{W} $$ is given by :

$$
\begin{aligned}
\operatorname{var}\left(\mathbf{y}_{1} \mid \mathbf{x}_{2}\right)=\operatorname{Var}(\mathbf{W}) &=\operatorname{Var}\left(\mathbf{y}_{1}+\mathbf{C} \mathbf{y}_{2}\right) \\
