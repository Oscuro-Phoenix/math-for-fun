...
The Barycentric coordinates can be considered to be the weights corresponding to how close the given point is to one of the three ordered vertices of the triange ABC. In fact, for a given position vector $$ \overrightarrow{P} $$ inside the triangle ABC with position vectors $$ \overrightarrow{A} $$, $$ \overrightarrow{B} $$ and $$ \overrightarrow{C} $$, we the Barycentric coordinates of P are (x,y,z) if :
