...
The term on the left hand side is some sort of a metric which says if the point is well-classified or not. If the inequality is satisfied for all points then we have found one possible hyperplane. But, which hyperplane is the best ? That would obviously be decided by the extent of separation between the data-points. So, we want the LHS of the last inequality to be as large as possible! In order to ensure that the magnitude of the vector $$ w $$ does not change our metric (it only depends on the direction). The obvious thing is to normalize it i.e.

$$ F\textsubscript{i} = y\textsubscript{i}.(\frac{w^T.X_i}{||w||} + \frac{b}{||w||}) $$ where $$ ||w|| $$ is the magnitude.

We need to maximise $$ \min\textsubscript{{x\textsubscript{1},\dots x\textsubscript{m}}} F\textsubscript{i} $$ under the constraint that $$ F\textsubscript{i} >= 1 $$. Quickly noting that the $$ \min\textsubscript{{x\textsubscript{1}},\dots x\textsubscript{m}} F\textsubscript{i} = \frac{1}{||w||} $$, our problem transforms into maximising $$ \frac{1}{||w||} $$ under the constrain that $$ F_i >= 1 $$
