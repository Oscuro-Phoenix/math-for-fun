...
We need to maximise the $$ \min_{x_1,\dots x_m} F_i $$ under the constraint that $$ F_i >= 1 $$. Quickly noting that the $$ \min_{x_1,\dots x_m} F_i = \frac{1}{||w||} $$, our problem transforms into maximising $$ \frac{1}{||w||} $$ under the constrain that $$ F_i >= 1 $$
