...
Let us define a sequence of functions of a complex number $$Z$$, $$\{f_0, f_1, f_2 \dots f_{i-1}, f_i, f_{i+1}, \dots \} $$ and try to draw an analogy to between $$f_{i-1},f_i, f_{i+1} \dots$$ and the sequence $$x,y,z \dots$$. Now let us say in our stick example the stick $$y$$ is just a little shorter that $$x$$ so that $$\floor{x/y} = a_1 = 1$$ then,
...
This is where my limitations exist. I'm not a genius like Gauss or Euler so I could never have figured this out ([See here](https://en.wikipedia.org/wiki/Gauss%27s_continued_fraction)). You probably need to spend enough time in the mess to see the patterns emerge but magically if one considers $$\Delta = k_i Z f_{i+1}$$ (to me this is like the "next" simplest thing to do after assuming $$\Delta \propto f_{i+1}$$) gives an interesting continued fraction recurrence relationship given by,
